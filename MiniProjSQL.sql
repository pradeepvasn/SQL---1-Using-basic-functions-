...
select bidder_id, (count(if(bid_status like 'Won',1,null))/count(bidder_id))*100 win_pct from ipl_bidding_details
group by bidder_id order by win_pct desc;


#2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select ism.stadium_name,ism.city,count(if(ims.status like 'Completed',1,null))as Tot_matches from ipl_stadium as ism join ipl_match_schedule
ims on ism.stadium_id=ims.stadium_id group by ism.stadium_id order by Tot_matches desc; 

#3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select t1.stadium_id,t1.stadium_name,t1.matches_played,t2.toss_won_matches_won, 
round(t2.toss_won_matches_won/t1.matches_played*100,2) won_pct
from 
(select s.stadium_id,s.stadium_name,count(m.stadium_id) matches_played
from ipl_stadium s join ipl_match_schedule m
on s.stadium_id=m.stadium_id
group by stadium_id)t1 join
(select stadium_id,count(stadium_id) toss_won_matches_won from 
(select s.stadium_id,m.match_id,m.team_id1,m.team_id2,m.Toss_winner,m.Match_Winner from ipl_match_schedule s 
join ipl_match m
on s.match_id=m.match_id
where toss_winner=match_winner)t
group by stadium_id)t2
